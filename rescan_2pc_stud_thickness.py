...
print('=' * 80)
print('RESCANNING 2PC STUD FILES WITH THICKNESS ERRORS')
print('=' * 80)
print()

# Find all 2PC STUD files
cursor.execute('''
...
print(f'Found {len(results)} 2PC STUD files\n')
...
for i, (prog_num, file_path, title, old_thickness, old_hub, old_status, old_issues) in enumerate(results):
    ...
    try:
        ...
        if parse_result:
            ...
            if status_improved or hub_was_populated:
                fixed_count += 1
                ...
                if fixed_count <= 30:
                    print(f'[{"FIXED" if status_improved else "UPDATED"}] {prog_num}: {title[:60]}')
                    print(f'  Thickness: {old_thickness}"')
                    if hub_was_populated:
                        print(f'  Hub Height: None -> {parse_result.hub_height:.3f}" (calculated)')
                    if status_improved:
...
print(f'  Status improved or hub populated: {fixed_count}')
print(f'  Hub height populated: {hub_populated_count}')
print(f'  Errors: {error_count}')
print()
print('Fixes applied:')
print('  - 2PC STUD with hub in title: Use hub_height from title')
print('  - 2PC STUD without hub: Calculate hub from drill depth')
print('  - STUD pattern detection: drill - thickness - 0.15 = 0.15-0.35" hub')
print('  - Hub height now populated for STUD parts')
print()
